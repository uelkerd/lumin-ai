    ...
    if (!data || !svgRef.cu  useEffect(() => {
      ...
      const drawChart = (width: number, height: number) => {
              ...
              if (tooltipRef.current) {
                ...
                tooltip.html(
                  `Series: ${d.series}<br/>` +
                    ...
                    `CI: [${d.confidence_interval[0].toFixed(2)}, ${d.confidence_interval[1].toFixed(
                      2,
                    )}]`,
                );
              }
        ...
        const legendItems = legend
          ...
          .on("click", (_event, seriesName: string) => {
            visibilityState[seriesName] = !visibilityState[seriesName];
            updateChart(d3.zoomTransform(svg.node() as SVGSVGElement));
          });
