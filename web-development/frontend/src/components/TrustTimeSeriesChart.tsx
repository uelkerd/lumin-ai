...
const TrustTimeSeriesChart: React.FC<TrustTimeSeriesChartProps> = ({
  data,
}) => {
  ...
  useEffect(() => {
    ...
    const area = d3
      .area<{ year: number; ci: [number, number] }>() // Use a simplified type for area generator
      .x((d) => xScale(d.year))
      .y0((d) => yScale(d.ci[0]))
      .y1((d) => yScale(d.ci[1]));
    ...
    // Define the line
    const line = d3
      .line<{ year: number; trust_score: number }>() // Use a simplified type for line generator
      .x((d) => xScale(d.year))
    ...
    const drawChart = (transform: d3.ZoomTransform) => {
      ...
      seriesGroups
        ...
        ) // Bind simplified data for area
        .attr(
          "fill",
          (d) =>
            colorScale(
              d3.select(d3.select(this).node()?.parentNode as any).datum()
                ?.key as string,
            ) as string,
        )
        .attr("opacity", 0.3)
