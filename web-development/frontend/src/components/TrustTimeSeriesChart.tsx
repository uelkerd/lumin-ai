...
const TrustTimeSeriesChart: React.FC = () => {
  ...
  const maxValue = Math.max(
    ...data.map((d) =>
      Math.max(d.trust, d.transparency, d.participation, d.accountability),
    ),
  );
  const minValue = Math.min(
    ...data.map((d) =>
      Math.min(d.trust, d.transparency, d.participation, d.accountability),
    ),
  );
    ...
    <div className="w-full">
      ...
      <div className="flex flex-wrap gap-sm mb-lg justify-center">
        {metrics.map((metric) => (
          <button
            ...
            style={
              selectedMetric === metric.key ? {} : { borderColor: metric.color }
            }
          >
            <span
      ...
      <div className="relative" style={{ height: "300px" }}>
        ...
        >
          ...
          {/* Y-axis labels */}
          {[0, 1, 2, 3, 4].map((i) => {
            const value = (maxValue - (maxValue - minValue) * (i / 4)).toFixed(
              1,
            );
            return (
              <text
              ...
              metrics.map((metric) => {
                const points = data
                  .map((point, index) => {
                    ...
                    const y =
                      50 +
                      ((maxValue - point[metric.key as keyof DataPoint]) /
                        (maxValue - minValue)) *
                        200;
                    return `${x},${y}`;
                  })
              ...
              (() => {
                ...
                const points = data
                  .map((point, index) => {
                    ...
                    const y =
                      50 +
                      ((maxValue - point[selectedMetric as keyof DataPoint]) /
                        (maxValue - minValue)) *
                        200;
                    return `${x},${y}`;
                  })
                  ...
                  <>
                    ...
                    <defs>
                      ...
                      >
                        <stop
                          offset="0%"
                          stopColor={selectedMetricData.color}
                          stopOpacity="0.3"
                        />
                        <stop
                          offset="100%"
        ...
        <div className="grid grid-cols-3 gap-lg text-center">
          <div>
            <div className="text-lg font-semibold gradient-text">
              {data.length > 0
                ? data[data.length - 1][selectedMetric as keyof DataPoint]
                : "--"}
            </div>
          ...
          <div>
            <div className="text-lg font-semibold" style={{ color: "#10B981" }}>
              ...
              {data.length > 0
                ? (
                    ((data[data.length - 1][selectedMetric as keyof DataPoint] -
                      data[0][selectedMetric as keyof DataPoint]) *
                      100) /
                    data[0][selectedMetric as keyof DataPoint]
                  ).toFixed(1)
                : 0}
            ...
            <div className="text-lg font-semibold text-blue-electric">
              {data.length > 0
                ? (
                    data.reduce(
                      (sum, point) =>
                        sum + point[selectedMetric as keyof DataPoint],
                      0,
                    ) / data.length
                  ).toFixed(1)
                : "--"}
