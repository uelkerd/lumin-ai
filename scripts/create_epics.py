...
def main():
    ...
    parser.add_argument("prd_file", help="Path to PRD.md file or directory containing PRD files")
    ...
    parser.add_argument(
        "--dry-run", action="store_true", help="Preview epics without creating them"
    )
    parser.add_argument(
        "--no-project", action="store_true", help="Skip adding epics to project board"
    )
    ...
    for prd_file in prd_files:
        try:
            ...
            if not args.dry_run:
                created = create_github_epics(client, epics, args.dry_run, not args.no_project)
