...
def parse_markdown_to_issues(file_path: str) -> List[Dict]:
    ...
    labels = [
        ...
        if part and part not in ["prd", "md", "mvp", "roadmap"]
    ]
    ...
    for line in lines:
        ...
        if match := h3_match or h2_match:
            ...
            current_issue = {"title": title, "body": [], "labels": labels}
        elif current_issue:
...
def main():
    ...
    parser.add_argument(
        "prd_directory", help="Path to the directory containing PRD markdown files."
    )
    parser.add_argument(
        "--output-file",
        default="issues.json",
    ...
    logger.info(
        f"python scripts/gh_issue_creator.py {args.output_file} --token YOUR_TOKEN --dry-run"
    )
